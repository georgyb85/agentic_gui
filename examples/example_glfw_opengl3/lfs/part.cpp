   ...
   )
{
   ...
   // Allocate memory using modern C++ unique_ptr
   x = std::make_unique<double[]>(n);
   ix = std::make_unique<int[]>(n);
   indices = std::make_unique<int[]>(n);
   bin_end = std::make_unique<int[]>(np);
...
*/
   ...
   qsortdsi ( 0 , n-1 , x.get() , indices.get() ) ;
...
*/
   ...
   for (;;) {
      ...
      for (ibound=0 ; ibound<np-1 ; ibound++)
         printf ( "\n  %2d=%d  (%d %d)",
                  ibound, bin_end[ibound], ix[bin_end[ibound]], ix[bin_end[ibound]+1] ) ;
#endif
...
*/
   ...
   *npart = np ;   // Return the final number of partitions
   ...
   for (ibound=0 ; ibound<np ; ibound++)
      printf ( "\n  %2d=%d  (%d %d) = %.5lf",
               ibound, bin_end[ibound], ix[bin_end[ibound]],
               ix[bin_end[ibound]+1], x[bin_end[ibound]] ) ;
#endif
...
*/
   ...
   // Memory automatically freed by unique_ptr destructors
