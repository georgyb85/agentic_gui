...
int main(int argc, char** argv) {
    ...
    std::cout << "====================================================================\n";
    std::cout << "PRICE CHANGE OSCILLATOR (PCO) VALIDATION\n";
    std::cout << "====================================================================\n\n";
    ...
    // Test PCO_10_20
    {
        std::cout << "Testing PCO_10_20 (params: 10, 20)\n";
        std::cout << "--------------------------------------------------------------------\n";

        // Get expected values from TSSB
        auto expected = TSBBOutputParser::extract_indicator_aligned(tssb_bars, ohlcv_bars, "PCO_10_20");

        // Compute our values
        SingleIndicatorRequest req;
        req.id = SingleIndicatorId::PriceChangeOscillator;
        req.name = "PCO_10_20";
        req.params[0] = 10;
        req.params[1] = 20;
        ...
        if (!result.success) {
            std::cerr << "ERROR computing PCO_10_20: " << result.error_message << "\n";
            return 1;
        }
        ...
        std::cout << "First 20 bars comparison:\n";
        ...
        for (int j = 0; j < 20 && first_valid + j < ohlcv_bars.size(); ++j) {
