...
// Modified version that tests CUBIC trend with custom c value
IndicatorResult compute_cubic_custom_c(const SingleMarketSeries& series, int lookback, int atr_length, double c_value) {
    ...
    const double* coefs = c3.data();  // CUBIC trend
...
void test_c_value(const SingleMarketSeries& series, const std::vector<OHLCVBar>& ohlcv_bars,
    ...
    auto result = compute_cubic_custom_c(series, lookback, atr_length, c_value);
...
int main(int argc, char** argv) {
    ...
    std::cout << "Testing different c values for CUBIC_TREND indicators:\n\n";

    std::cout << "CUBIC_TREND_S (lookback=10, atr=100):\n";
    for (double c = 1.0; c <= 20.0; c += 1.0) {
        test_c_value(series, ohlcv_bars, tssb_bars, "CUBIC_TREND_S", 10, 100, c);
    }

    std::cout << "\nCUBIC_TREND_L (lookback=60, atr=250):\n";
    for (double c = 1.0; c <= 20.0; c += 1.0) {
        test_c_value(series, ohlcv_bars, tssb_bars, "CUBIC_TREND_L", 60, 250, c);
    }
