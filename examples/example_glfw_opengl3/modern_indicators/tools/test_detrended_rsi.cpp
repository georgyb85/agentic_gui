...
int main(int argc, char** argv) {
    ...
    std::cout << "Testing DETRENDED RSI variants\n";
    std::cout << "===============================\n\n";

    // DTR_RSI_M: DETRENDED RSI 5 20 100
    // DTR_RSI_L: DETRENDED RSI 5 20 200
    std::vector<std::tuple<std::string, int, int, int>> tests = {
        {"DTR_RSI_M", 5, 20, 100},
        {"DTR_RSI_L", 5, 20, 200}
    };
    ...
    for (const auto& [name, short_len, long_len, regression_len] : tests) {
        std::cout << "\n" << name << " (short=" << short_len << ", long=" << long_len << ", regression=" << regression_len << ")\n";
        std::cout << std::string(70, '-') << "\n";

        SingleIndicatorRequest req;
        req.id = SingleIndicatorId::DetrendedRsi;
        req.name = name;
        req.params[0] = short_len;
        req.params[1] = long_len;
        req.params[2] = regression_len;
